...
import (
	...
	"github.com/invopop/gobl.co.dian/addon"
...
func TestCreditNoteWithoutReferenceValidation(t *testing.T) {
	...
	t.Run("rejects periods crossing calendar months", func(t *testing.T) {
		inv := unreferenced()
		inv.Preceding[0].Period = &cal.Period{Start: cal.NewDate(2022, 11, 15), End: cal.NewDate(2022, 12, 15)}
		require.NoError(t, inv.Calculate())
		err := rules.Validate(inv)
	...
	t.Run("allows periods within one calendar month", func(t *testing.T) {
		inv := unreferenced()
		inv.Preceding[0].Period = &cal.Period{Start: cal.NewDate(2022, 12, 1), End: cal.NewDate(2022, 12, 28)}
		require.NoError(t, inv.Calculate())
		assert.NoError(t, rules.Validate(inv))
	})
